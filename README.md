...
The validator accepts only entries with known posting accounts, valid non-negative single-sided non-zero lines, a 2024-Q4 date, and balanced debits and credits within USD 0.01. Suspicious same-account patterns are escalated; failed entries are rejected with reasons. Raw input is never rewritten.
...
The JSON audit file also includes run metadata, source file hashes, structured rule codes, and line-level validation detail for every journal line.
